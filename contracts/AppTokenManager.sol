...
contract AppTokenManager is ProxyFactory {    
    ...
    function createAppToken(string memory _name, string memory _symbol, uint256 _amount, address _owner, address _propsOwner) public returns (address){
        bytes memory payload = abi.encodeWithSignature("initialize(string,string,uint256,address,address)", _name, _symbol, _amount, _owner, _propsOwner);
