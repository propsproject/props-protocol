...
DEPLOY=1 npx hardhat --network $L1_NETWORK run scripts/deploy-l1.ts
printf "${COLOR}Deploying L2 contracts on ${L2_NETWORK}...${CLEAR}\n"
DEPLOY=1 npx hardhat --network $L2_NETWORK run scripts/deploy-l2.ts

# Connect L1 - L2 bridges
printf "${COLOR}Connecting L1 contracts to L2...${CLEAR}\n"
CONNECT=1 npx hardhat --network $L1_NETWORK run scripts/deploy-l1.ts
printf "${COLOR}Connecting L2 contracts to L1...${CLEAR}\n"
CONNECT=1 npx hardhat --network $L2_NETWORK run scripts/deploy-l2.ts